...
// 展开为数组(原生nodelist是类似数组的对象即对象中有length属性)
[...document.querySelectorAll('*')] // 转换为真正的数组
// 遍历数组
...
```
[数组的扩展 - ECMAScript 6入门](http://es6.ruanyifeng.com/#docs/array#%E6%89%A9%E5%B1%95%E8%BF%90%E7%AE%97%E7%AC%A6)
...
[...document.querySelectorAll('p')].filter(
    ...
    ).map(p=>{ return p.textContent}).join('')
```

>上面代码中，querySelectorAll方法返回的是一个类似数组的对象，可以将这个对象转为真正的数组，
>再使用filter方法。
...
```
这个代码中重点在  this.shopCartStickyComp = this.shopCartStickyComp || this.$createShopCartSticky({
如果this.shopCartStickyComp 中有值则不用创建
